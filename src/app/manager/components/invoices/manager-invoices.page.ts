...
export class ManagerInvoicesPageComponent implements OnInit, OnDestroy {
    ...
    forkJoin({
      ...
      .subscribe({
        next: (res) => {
          ...
          this.selectedInvoice = res.document?.document ?? doc;
          this.replaceInvoice(this.selectedInvoice);
  ...
  private replaceInvoice(document: BmDocument | null): void {
    if (!document?.documentId) return;
    this.filteredInvoices$.next(
      this.filteredInvoices$.value.map((item) =>
